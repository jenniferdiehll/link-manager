...
const generateJwt = (payload) => {//Function que gera o token
    return jwt.sign(payload, tokenPrivateKey, options);
};

const generateRefreshJwt = (payload) => {//Function que gera o token
    return jwt.sign(payload, refreshTokenPrivateKey, refreshOptions);
};

const verifyJwt = (token) => {//Function que verifica o token
    return jwt.verify(token, tokenPrivateKey);
};

const verifyRefreshJwt = (token) => {//Function que verifica o token
